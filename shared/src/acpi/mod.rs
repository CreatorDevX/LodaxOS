...
/// If `rsdp_addr` is Some, use it directly (e.g., from UEFI config table);
/// if None, scan memory for the RSDP (fallback for non-UEFI environments).
...
/// Find a system description table by signature within the XSDT (8-byte entries)
/// or RSDT (4-byte entries). Detects the entry size from the table signature.
